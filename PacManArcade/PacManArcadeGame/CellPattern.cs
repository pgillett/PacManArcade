            ...
            for (int y = -1; y < 2; y++)
            {
                for (int x = -1; x < 2; x++)
                {
                    ...
                    if (compare == 'W' && onBoard.BasicType != BasicMapPiece.SingleWall
                                       && onBoard.BasicType != BasicMapPiece.DoubleWall) return false;

                    if (compare == '0' && onBoard.BasicType != BasicMapPiece.PlayArea
                                       && onBoard.BasicType != BasicMapPiece.ThroughSpace
                                       && onBoard.BasicType != BasicMapPiece.ThroughSpacePill
                                       && onBoard.BasicType != BasicMapPiece.Tunnel
                                       && onBoard.BasicType != BasicMapPiece.Pill) return false;

                    if (compare == '#' && onBoard.BasicType != BasicMapPiece.DoubleWall) return false;

                    if (compare == '~' && onBoard.BasicType != BasicMapPiece.DeadSpace) return false;
                }
            }
